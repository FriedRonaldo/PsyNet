...
def rescamtf(resconfig, tftypes, tfnums, pretrained, **kwargs):
    if resconfig == 'rescam18':
        model = resnet18(pretrained, tftypes=tftypes, tfnums=tfnums, **kwargs)
    elif resconfig == 'rescam34':
        model = resnet34(pretrained, tftypes=tftypes, tfnums=tfnums, **kwargs)
    elif resconfig == 'rescam50':
        model = resnet50(pretrained, tftypes=tftypes, tfnums=tfnums, **kwargs)
    elif resconfig == 'rescam101':
        model = resnet101(pretrained, tftypes=tftypes, tfnums=tfnums, **kwargs)
    elif resconfig == 'rescam152':
        model = resnet152(pretrained, tftypes=tftypes, tfnums=tfnums, **kwargs)
    else:
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        pretrained_dict = torch.utils.model_zoo.load_url(model_urls[arch], progress=progress)
