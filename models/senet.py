...
class SENetTF(nn.Module):
    ...
    def logits(self, x):
        ...
        for block in self.blocks:
            cam = block(x)
            cams.append(cam)
            logit = self.avg_pool(cam).squeeze()
            logits.append(logit)
        cams.append(chatt)
        return logits, cams
...
def serescamtf(resconfig, tftypes, tfnums, pretrained, **kwargs):
...
def _seresnet(block, layers, pretrained, groups, reduction,
              inplanes, input_3x3, downsample_kernel_size,
              downsample_padding, resconfig, tftypes, tfnums, **kwargs):
    model = SENetTF(block, layers, groups, reduction, None, inplanes,
                    input_3x3, downsample_kernel_size, downsample_padding,
                    tftypes, tfnums)
    if pretrained:
        print('USE PRETRAINED BACKBONE')
        model_dict = model.state_dict()

        pretrained_dict = model_zoo.load_url(pretrained_settings[resconfig]['imagenet']['url'])
